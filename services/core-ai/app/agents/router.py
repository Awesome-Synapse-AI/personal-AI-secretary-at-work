...
MAIN_ROUTE_PROMPT = (
    ...
    "request means user is asking to submit, change, or execute an operational action now. "
    ...
    "Use request only when the user wants the assistant to take action now (submit/file/book/create/update/cancel/approve/reject a request). "
    "Classification priority: explicit action verbs (submit, book, reserve, file, create, request access, approve, reject, cancel) "
    "should outweigh topical words like policy/rule unless the user clearly asks for explanation only. "
    "Domain hints for request: "
    "HR request actions include leave submission/approval. "
    "Ops request actions include expense submission and travel arrangement. "
    "IT request actions include ticket creation and access requests. "
    "Workspace request actions include booking rooms/desks/equipment/parking. "
    "If unsure, use generic and normal. "
    "Examples:\n"
    "Input: I need sick leave tomorrow -> {\"main_route\":\"request\",\"sensitivity\":\"hr_personal\"}\n"
    "Input: Approve leave request #42 -> {\"main_route\":\"request\",\"sensitivity\":\"hr_personal\"}\n"
    "Input: Reimburse taxi fare from yesterday -> {\"main_route\":\"request\",\"sensitivity\":\"normal\"}\n"
    "Input: Submit expense for hotel invoice 250 USD -> {\"main_route\":\"request\",\"sensitivity\":\"normal\"}\n"
    "Input: Book a meeting room at 2pm -> {\"main_route\":\"request\",\"sensitivity\":\"normal\"}\n"
    "Input: Reserve projector for 4pm in Zephyr room -> {\"main_route\":\"request\",\"sensitivity\":\"normal\"}\n"
    "Input: Open an IT ticket for VPN disconnection -> {\"main_route\":\"request\",\"sensitivity\":\"access\"}\n"
    "Input: Grant editor access to analytics repo -> {\"main_route\":\"request\",\"sensitivity\":\"access\"}\n"
    "Input: Explain compensation and benefits -> {\"main_route\":\"doc_qa\",\"sensitivity\":\"normal\"}\n"
    "Input: How many sick leave days are allowed per year? -> {\"main_route\":\"doc_qa\",\"sensitivity\":\"normal\"}\n"
    ...
    "Input: How do I request VPN access? -> {\"main_route\":\"doc_qa\",\"sensitivity\":\"access\"}\n"
...
REQUEST_DOMAIN_PROMPT = (
    ...
    "HR: leave submission/approval/rejection and entitlement actions. "
    "OPS: expense/travel actions, including transport for client visits and trip planning. "
    "IT: access requests and issue tickets (including facilities incidents routed via ticket flow). "
    "WORKSPACE: only office resource reservations (room/desk/equipment/parking). "
    "travel/transport requests for client visits or trips belong to ops, "
    "while workspace is only for office resource booking (room/desk/equipment/parking). "
    "If user asks to 'book/reserve room/desk/equipment/parking', prefer workspace. "
    "If user asks to 'book travel/flight/train/car to customer/client site', prefer ops. "
    "Examples:\n"
    "Input: I need sick leave tomorrow -> {\"request_domain\":\"hr\"}\n"
    "Input: Approve leave request 15 -> {\"request_domain\":\"hr\"}\n"
    ...
    "Input: Submit travel request from Bangkok to Singapore next Monday -> {\"request_domain\":\"ops\"}\n"
    "Input: Grant me viewer access to data lake -> {\"request_domain\":\"it\"}\n"
    "Input: File ticket: AC in room 12 not working -> {\"request_domain\":\"it\"}\n"
    "Input: VPN keeps dropping -> {\"request_domain\":\"it\"}\n"
    "Input: Reserve room A for 3pm -> {\"request_domain\":\"workspace\"}\n"
    "Input: Reserve Zephyr room on 16/Mar/2026 from 9:00 to 11:00 -> {\"request_domain\":\"workspace\"}\n"
    "Input: Reserve projector for 3pm -> {\"request_domain\":\"workspace\"}\n"
)
